  ...
  return (
    <div className="rounded-lg border border-[#e5e5e5] bg-[#fafafa] p-5">
      <div className="mb-1 flex items-center justify-between">
        <span className="font-semibold text-base text-foreground">Online Now</span>
        <div className="flex items-center gap-1.5">
          <span className="size-2 rounded-full bg-green-500" />
          <span className="text-muted-foreground text-xs">{onlineMembers.length} online</span>
        </div>
      </div>
      <div className="flex flex-col">
        {onlineMembers.map((m) => (
          <div key={m.id} className="flex items-center gap-2.5 py-2">
            ...
            <span className="flex-1 font-medium text-foreground text-sm">{m.name}</span>
            {m.role !== "member" && (
